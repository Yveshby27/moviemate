...
export const userRouter = createTRPCRouter({
    ...
    createUser: publicProcedure
      .input(z.object({newUserId:z.string(), email: z.string(), password: z.string(),isAdmin:z.boolean() }))
      .mutation(async (obj) => {
        const newUser=await createUser({newUserId:obj.input.newUserId,email: obj.input.email, password: obj.input.password,isAdmin:obj.input.isAdmin });
        return {
          message: `User added to database(TRPC).`,data:newUser
        };
      }),
    getSpecifiedUser: publicProcedure.input(z.string()).query(async (arg) => {
      const specifiedUser= await fetchSpecifiedUser(arg.input);
